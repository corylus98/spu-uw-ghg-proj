...
export default function DataLabeling({
    ...
    folders.forEach(folder => {
      folder.files.forEach(file => {
        if (!newStatuses[file.id]) {
          // New file - set to Pending initially
          newStatuses[file.id] = 'Pending';
          hasChanges = true;
        } else if (newStatuses[file.id] === 'Processing' && file.id !== selectedFileId) {
          // File is Processing but not selected - check if it has column selections
          const hasColumnSelections = fileColumnLabels[file.id] && Object.keys(fileColumnLabels[file.id]).length > 0;
          newStatuses[file.id] = hasColumnSelections ? 'Completed' : 'Pending';
          hasChanges = true;
        }
        // Don't change status for Completed or Pending files - they keep their status
      });
    });
    if (hasChanges) {
  ...
  // Click timer ref for distinguishing single vs double click
  const clickTimerRef = useRef(null);

  // Handle column click - open Data Title modal for new selection (delayed to allow double-click)
  ...
  // Handle column double-click - quick select with original title
  const handleColumnDoubleClick = (columnIndex) => {
    ...
    if (selectedColumns[columnIndex]) return; // Already selected

    const originalTitle = selectedFileData?.headers[columnIndex] || `Column ${columnIndex + 1}`;
    const newSelectedColumns = {
      ...selectedColumns,
      [columnIndex]: originalTitle
    };
  ...
  const handleDeselectColumn = (columnIndex) => {
    ...
    if (selectedFileId) {
      ...
      // Clear pending corrections for this column in this file
      setFilePendingCorrections(prev => {
        const newPending = { ...prev };
        if (newPending[selectedFileId]) {
          const filePending = { ...newPending[selectedFileId] };
          // Remove all corrections in this column
          Object.keys(filePending).forEach(rowIdx => {
            if (filePending[rowIdx] && filePending[rowIdx][columnIndex]) {
              const rowPending = { ...filePending[rowIdx] };
              delete rowPending[columnIndex];
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      setLeftPanelWidth((clampedPx / rect.width) * 100);
    };
        ...
        <div className="flex-1 min-w-0 h-full pt-[24px] pb-[24px] pl-[12px] relative flex flex-col">
          ...
          <div className="flex items-start justify-between mb-[24px]">
            ...
            <p className="font-['Noto_Sans',sans-serif] font-normal text-[14px] text-[#7E7E7E] max-w-[360px] text-right leading-relaxed h-[39px] flex items-center justify-end">
              Select a file from the left panel to preview.<br/>Click on column headers to mark the data you need.
            </p>
          </div>
          ...
          {selectedFileId ? (
            <>
                              ...
                              <tr key={rowIdx}>
                                {row.map((cell, cellIdx) => (
                                  <td
                                    ...
                                    onDoubleClick={(e) => {
                                      e.stopPropagation();
                                      handleColumnDoubleClick(cellIdx);
                                    }}
              ...
              <div className="mt-[24px]">
                ...
                <div className="flex flex-wrap gap-[8px] max-h-[74px] overflow-y-auto pr-[4px]">
