...
import { listSources, previewSource, uploadFile } from './services/api';
...
export default function Input({ folders, setFolders, onComplete, onGoToStep }) {
  ...
  // SharePoint loading state
  const [loadingSharePoint, setLoadingSharePoint] = useState(false);

  // Preview state - for showing data when a file is clicked
  ...
  const handleUploadConfirm = () => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        // Determine S3 folder from tag (#Consumption → CONSUMPTION, #Reference → REFERENCE)
        const s3Folder = fileTag === '#Reference' ? 'REFERENCE' : 'CONSUMPTION';

        // Upload to backend (S3 in cloud, local dir in local mode)
        let backendSourceId = null;
        try {
          const uploadResult = await uploadFile(selectedFile, s3Folder);
          backendSourceId = uploadResult.sourceId;
        } catch (uploadError) {
          console.error('Backend upload failed:', uploadError);
          // Continue anyway so the file is still usable locally in the session
        }
        ...
        const newFile = {
          ...
          content: parsedData,
          sourceId: backendSourceId,  // used by downstream pages for API calls
        };
  ...
  // Handle SharePoint import - only fetch source list, no preview calls
  const handleSharePointImport = async () => {
    setLoadingSharePoint(true);
    try {
      const result = await listSources();
      const consumptionSources = result.sources || [];
      const referenceSources = result.reference || [];

      const toFile = (src) => ({
        id: src.sourceId,
        name: src.name,
        originalName: src.sourceId,
        sourceId: src.sourceId,
        isSharePoint: true,
        content: null, // loaded lazily on click via preview API
      });

      const consumptionFiles = consumptionSources.map(toFile);
      const referenceFiles = referenceSources.map(toFile);

      setFolders(prev => prev.map(folder => {
        if (folder.tag === '#Consumption') {
          const existingIds = new Set(folder.files.map(f => f.id));
          const newFiles = consumptionFiles.filter(f => !existingIds.has(f.id));
          return { ...folder, files: [...folder.files, ...newFiles] };
        }
        if (folder.tag === '#Reference') {
          const existingIds = new Set(folder.files.map(f => f.id));
          const newFiles = referenceFiles.filter(f => !existingIds.has(f.id));
          return { ...folder, files: [...folder.files, ...newFiles] };
        }
        return folder;
      }));
    } catch (err) {
      console.error('Failed to import from SharePoint:', err);
      alert('Failed to connect to backend. Make sure the backend is running on port 8000.');
    } finally {
      setLoadingSharePoint(false);
    }
  };

  // Handle clicking a file in the staging area to load its preview
  const handleFileClick = async (file) => {
    if (selectedPreviewFileId === file.id) {
      // Toggle off
    ...
    // If it's a SharePoint file without content, fetch preview from backend
    if (file.isSharePoint && !file.content) {
      setLoadingPreview(true);
      setPreviewData(null);
      try {
        const result = await previewSource(file.sourceId);
        const headers = result.columns.map(c => c.name);
        const rows = (result.preview || []).map(row =>
          headers.map(h => (row[h] != null ? String(row[h]) : ''))
        );
        setPreviewData({ headers, rows, totalRows: result.totalRows, columns: result.columns });

        // Also store the content on the file so DataCleaning can use it later
        setFolders(prev => prev.map(folder => ({
          ...folder,
          files: folder.files.map(f =>
            f.id === file.id ? { ...f, content: { headers, rows } } : f
          ),
        })));
      } catch (err) {
        console.error('Failed to load preview:', err);
        setPreviewData({ error: 'Failed to load preview. Check that the backend is running.' });
      } finally {
        setLoadingPreview(false);
      }
    } else if (file.content) {
      // Local file - use its content directly
      setPreviewData({ headers: file.content.headers, rows: file.content.rows });
      setLoadingPreview(false);
    }
  };
  ...
  return (
    <div className="bg-white fixed inset-0 overflow-hidden" data-name="INPUT">
      ...
      <div className="absolute left-[64px] right-[64px] top-[132px] bottom-[32px] flex gap-[24px]">
                ...
                {folder.files.length > 0 ? (
                  folder.files.map((file) => (
                    <div
                      ...
                      className={`bg-white h-[49px] flex items-center pl-[17px] pr-[17px] justify-between border-t border-[#EEEEEE] relative hover:bg-[#29ABB5]/10 transition cursor-pointer ${
                        draggedFile?.file.id === file.id ? 'opacity-50' : ''
                      } ${selectedPreviewFileId === file.id ? 'bg-[#29ABB5]/10 border-l-4 border-l-[#29ABB5]' : ''}`}
                    >
                      <div className="flex items-center gap-[12px] pl-[32px]">
                        <DataTableIcon className={`w-[24px] h-[24px] ${selectedPreviewFileId === file.id ? 'text-[#29ABB5]' : 'text-[#7E7E7E]'}`} />
                        <p className={`font-['Noto_Sans',sans-serif] font-normal text-[14px] ${selectedPreviewFileId === file.id ? 'text-[#29ABB5] font-medium' : 'text-[#13383B]'}`}>
                          {file.name}
                        </p>
                      </div>
        ...
        {/* Right Column - Upload Area or Data Preview */}
        <div className="flex-1 h-full pt-[24px] pb-[24px] pl-[24px] relative flex flex-col">
          {selectedPreviewFileId ? (
            ...
            <>
              <div className="flex items-center justify-between mb-[16px]">
                ...
                <button
                  onClick={() => { setSelectedPreviewFileId(null); setPreviewData(null); }}
                  className="px-[16px] py-[6px] rounded-[8px] border border-[#D9D9D9] font-['Noto_Sans',sans-serif] text-[13px] text-[#575757] hover:bg-[#F5F5F5] transition"
                >
                  Back to Upload
                </button>
              </div>
              <p className="font-['Noto_Sans',sans-serif] font-normal text-[14px] text-[#7E7E7E] mb-[16px]">
                {folders.flatMap(f => f.files).find(f => f.id === selectedPreviewFileId)?.name || selectedPreviewFileId}
              </p>

              {/* Preview Table */}
              <div className="flex-1 overflow-hidden mb-[70px]">
                {loadingPreview ? (
                  <div className="flex items-center justify-center h-full bg-[#F5F5F5] rounded-[10px]">
                    <div className="flex flex-col items-center gap-[8px]">
                      <div className="animate-spin w-[32px] h-[32px] border-4 border-[#29ABB5] border-t-transparent rounded-full"></div>
                      <p className="font-['Noto_Sans',sans-serif] text-[14px] text-[#7E7E7E]">Loading preview...</p>
                    </div>
                  </div>
                ) : previewData?.error ? (
                  <div className="flex items-center justify-center h-full bg-[#F5F5F5] rounded-[10px]">
                    <p className="font-['Noto_Sans',sans-serif] text-[14px] text-[#DC3545]">{previewData.error}</p>
                  </div>
                ) : previewData?.headers ? (
                  <div className="border border-[#EEEEEE] rounded-[10px] overflow-hidden h-full flex flex-col">
                    ...
                    {previewData.totalRows && (
                      <div className="px-[12px] py-[8px] bg-[#F5F5F5] border-t border-[#EEEEEE] flex-shrink-0">
                  ...
                  <div className="flex items-center justify-center h-full bg-[#F5F5F5] rounded-[10px]">
                    <p className="font-['Noto_Sans',sans-serif] text-[14px] text-[#7E7E7E]">No data available</p>
                  </div>
                )}
              </div>
            </>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-[12px] mb-[24px]">
                ...
                <div
                  onClick={loadingSharePoint ? undefined : handleSharePointImport}
                  className={`h-[213px] rounded-[10px] shadow-[0px_2px_4px_0px_rgba(0,0,0,0.15)] transition ${
                    loadingSharePoint
                      ? 'bg-[#EEEEEE] cursor-wait'
                      : 'bg-white border-2 border-[#365D60] cursor-pointer hover:bg-[#f0f5f5]'
                  }`}
                >
                  <div className="h-[176px] flex items-center justify-center">
                    {loadingSharePoint ? (
                      <div className="flex flex-col items-center gap-[8px]">
                        <div className="animate-spin w-[32px] h-[32px] border-3 border-[#365D60] border-t-transparent rounded-full"></div>
                        <p className="font-['Noto_Sans',sans-serif] text-[12px] text-[#7E7E7E]">Loading...</p>
                      </div>
                    ) : (
                      <SharePointIcon className="w-[44px] h-[44px] text-[#365D60]" />
                    )}
                  </div>
                  <div className={`h-[37px] rounded-bl-[10px] rounded-br-[10px] flex items-center justify-center ${
                    loadingSharePoint ? 'bg-[#CCCCCC]' : 'bg-[#365D60]'
                  }`}>
                    <p className={`font-['Noto_Sans',sans-serif] font-normal text-[14px] ${
                      loadingSharePoint ? 'text-[#7E7E7E]' : 'text-white'
                    }`}>
          ...
          {/* Upload Completed Button - always at bottom */}
